...
export async function makeAlert(c: Context<{ Bindings: { DATABASE_URL: string } }>) {
...
export async function getAlerts(c: Context<{ Bindings: { DATABASE_URL: string } }>) {
    const prisma = getPrisma(c.env.DATABASE_URL);
    const alerts = await prisma.alert.findMany();
    return c.json(alerts);
}

export async function deleteAlert(c: Context<{ Bindings: { DATABASE_URL: string } }>) {
    const prisma = getPrisma(c.env.DATABASE_URL);
...
export async function updateAlert(c: Context<{ Bindings: { DATABASE_URL: string } }>) {
    const prisma = getPrisma(c.env.DATABASE_URL);
    const { userEmail, mint, type, comparison, threshold } = await c.req.json();
    await prisma.alert.update({ where: { userEmail }, data: { mint, parameter: type, comparison, threshold } });
    return c.json({ message: "Alert updated successfully" });
}

export async function getAlertByUserEmail(c: Context<{ Bindings: { DATABASE_URL: string } }>) {
    const prisma = getPrisma(c.env.DATABASE_URL);
    const { userEmail } = await c.req.json();
...
export async function getAlertByMint(c: Context<{ Bindings: { DATABASE_URL: string } }>) {
    const prisma = getPrisma(c.env.DATABASE_URL);
